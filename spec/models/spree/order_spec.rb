describe Spree::Order do
  ...
  context '.process_payments!' do
    ...
    it 'processes payment when total is zero and payments is not empty' do
      allow(order).to receive(:unprocessed_payments).and_return([mock_model(Spree::Payment)])
  ...
  describe 'when minimum item total is set' do
    ...
    context 'when item total is greater than limit' do
      before do
        reset_spree_preferences { |config| config.use_store_credit_minimum = 10 }
      end
